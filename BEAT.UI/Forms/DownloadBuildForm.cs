...
namespace BEAT.UI.Forms
{
    ...
    /// DownloadBuildForm
    /// </summary>
    public partial class DownloadBuildForm : Form
    {
        #region Variables
        ...
        /// <param name="e"></param>
        private void DownloadBuildForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                textBoxLocalDirectoryPath.Text = ConfigurationManager.AppSettings["BPDiffPath"];
        ...
        private void BrowseFolder_Click(object sender, EventArgs e)
        {
            ...
            textBoxBuildDownloadPath.Text = folderBrowserDialog.SelectedPath; // <-- For debugging use only.
        }
        ...
        private void buttonVerifyAgainstBrowse_Click(object sender, EventArgs e)
        {
            ...
            textBoxFtpUserName.Text = folderBrowserDialog.SelectedPath; // <-- For debugging use only.
        ...
        private void buttonBPDiffBrowse_Click(object sender, EventArgs e)
        {
            ...
            textBoxLocalDirectoryPath.Text = folderBrowserDialog.SelectedPath; // <-- For debugging use only.
        }
        ...
        private void buttonEnglishRefBrowse_Click(object sender, EventArgs e)
        {
            ...
            textBoxftpPassword.Text = openFileDialog.FileName; // <-- For debugging use only.
        }
        ...
        private void buttonSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!ValidateInputs(textBoxBuildDownloadPath.Text, textBoxFtpUserName.Text, textBoxftpPassword.Text))
                    return;

                if (!ValidateOutputs(textBoxLocalDirectoryPath.Text, textBoxBPResults.Text))
                    return;
        ...
        private void FileandFoldersDataBPComparison()
        {
            ...
            {
                BPComparison(textBoxBuildDownloadPath.Text, textBoxFtpUserName.Text, textBoxftpPassword.Text, textBoxLocalDirectoryPath.Text);
            }
            else if (IsVerifyBPFolder)
            {
                DirectoryInfo dir = new DirectoryInfo(textBoxBuildDownloadPath.Text);
                ...
                foreach (FileInfo file in files)
                {
                    if (IsFileCSV(file.Name))
                    {
                        if (IsVerifyAgainstBPFolder)
                            BPComparison(textBoxBuildDownloadPath.Text + "\\" + file.Name, textBoxFtpUserName.Text + "\\" + file.Name, textBoxftpPassword.Text, textBoxLocalDirectoryPath.Text + "\\" + file.Name.Replace(".csv", "_Diff.csv"));
                        else
                            BPComparison(textBoxBuildDownloadPath.Text + "\\" + file.Name, textBoxFtpUserName.Text, textBoxftpPassword.Text, textBoxLocalDirectoryPath.Text + "\\" + file.Name.Replace(".csv", "_Diff.csv"));
                    }
                }
            }
            else if (!IsVerifyBPFolder)
            {
                DirectoryInfo dir = new DirectoryInfo(textBoxFtpUserName.Text);
                FileInfo[] files = dir.GetFiles();
                ...
                foreach (FileInfo file in files)
                {
                    ...
                    {
                        if (IsVerifyAgainstBPFolder)
                            BPComparison(textBoxBuildDownloadPath.Text, textBoxFtpUserName.Text + "\\" + file.Name, textBoxftpPassword.Text, textBoxLocalDirectoryPath.Text + "\\" + file.Name.Replace(".csv", "_" + file.Name + "_Diff.csv"));
                    }
                }
            }
        ...
        private void XMLDataBPComparison(string dataXml, string verifyBPBuildNo, string verifyAgainstBPBuildNo, string englishRefBuildNo)
        {
            ...
            foreach (DataRow drXmlData in dtXmlData.Rows)
            {
                ...
                string bpDiffPath = textBoxLocalDirectoryPath.Text + drXmlData[2].ToString();
        ...
        private string GetEnglishBPRefPath(string englishBPRef, string fileName)
        {
            if (!IsFileCSV(englishBPRef))
            {
                ...
                textBoxftpPassword.Text = path.ToString();
        ...
        private void BPComparison(string fileNameVerifyBP, string fileNameVerifyAgainstBP, string fileNameEnglishBPRef, string fileNameBPDiff)
        {
            ...
            if (IsVerifyAgainstAnotherBP)
            {
                ...
                if (File.Exists(fileNameEnglishBPRef))
                {
                    ...
                    textBoxftpPassword.Text = fileNameEnglishBPRef;
                }
                else
                {
                    if (textBoxftpPassword.Text != "" && !textBoxftpPassword.Text.Contains("English Ref File doesn't exist."))
                    {
                        textBoxftpPassword.Text = "English Ref File doesn't exist. (" + textBoxftpPassword.Text + ")";
                    }
                }
                ...
                if (textBoxftpPassword.Text == "")
                    dr[1] = fileNameVerifyAgainstBP;
                else
                    dr[1] = fileNameVerifyAgainstBP + " / " + textBoxftpPassword.Text;
                ...
                if (dtEnglishBPRef != null)
                {
                    foreach (DataRow drEnglishBPRef in dtEnglishBPRef.Rows)
                    {
                        ...
                        if (dataRow.Length == 1)
                        {
                            ...
                            {
                                if ((Convert.ToInt32(drEnglishBPRef["FileSize"].ToString()) == Convert.ToInt32(dr["NewFileSize"].ToString())) && (drEnglishBPRef["FilePermissions"].ToString() == dr["NewFilePermissions"].ToString()) || (drEnglishBPRef["FileMD5Checksum"].ToString() == dr["NewFileMD5Checksum"].ToString()))
                                    dr[1] = "NoChange";
                                else
                                {
        ...
        private DataTable CreateDiffFile(DataTable dtVerifyBP, DataTable dtVerifyAgainstBP)
        {
            ...
            foreach (DataRow drVerifyBP in dtVerifyBP.Rows)
            {
                ...
                else
                {
                    try
                    {
                        if ((Convert.ToInt32(drVerifyBP[1].ToString()) != Convert.ToInt32(dataRow[0][1].ToString())) || (drVerifyBP[2].ToString() != dataRow[0][2].ToString()) || (drVerifyBP[3].ToString() != dataRow[0][3].ToString()))
                            dr[1] = "Modified";
                        else
                            dr[1] = "NoChange";
                ...
                dr[5] = drVerifyBP[1].ToString();
                dr[6] = drVerifyBP[2].ToString();
                dr[7] = drVerifyBP[3].ToString();
        ...
        {
            //DataTable data = BO.BusinessObjects.BEATBO.ReadExcludedZStringsfromCSV(file); ;
            //return data;
            return null;
        }
        ...
        private void WriteBPDiff(DataTable dtBPDiff, string fileNameBPDiff)
        {
            ...
            {
                sw.WriteLine("RelativeFilePath,Status,SizeDiff,FilePermissions,MD5CheckSum,NewFileSize,NewFilePermissions,NewFileMD5Checksum");
        ...
        private void EnableXMLFileRadioButton(bool isEnabled)
        {
            if (isEnabled)
            {
                ...
                textBoxFtpUserName.Enabled = false;
                textBoxBuildDownloadPath.Enabled = false;
                ...
                textBoxBuildDownloadPath.Text = "Build #" + ConfigurationManager.AppSettings["VerifyBPBuildNo"] + " [" + ConfigurationManager.AppSettings["XMLPath"] + "]";
                textBoxFtpUserName.Text = "Build #" + ConfigurationManager.AppSettings["VerifyAgainstBPBuildNo"] + " [" + ConfigurationManager.AppSettings["XMLPath"] + "]";
                ...
                textBoxftpPassword.Text = "Picked based on XML";
            }
        }
        ...
        private void EnableFolderFilesRadioButton(bool isEnabled)
        {
            if (isEnabled)
            {
                ...
                textBoxFtpUserName.Enabled = true;
                textBoxBuildDownloadPath.Enabled = true;
                ...
                textBoxBuildDownloadPath.Text = ConfigurationManager.AppSettings["VerifyBP"];
                textBoxFtpUserName.Text = ConfigurationManager.AppSettings["VerifyAgainstBP"];
                textBoxftpPassword.Text = ConfigurationManager.AppSettings["EnglishBPRef"];
            }
        }
